...
一个运行在本地 Mac 上的电脑智能体。用户在网页中输入目标任务后，程序会发送给多模态模型，由模型返回 JSON 操作计划，再由程序执行 `zsh`、观察、等待、鼠标键盘操作或向用户请求补充信息，达到帮助用户完成任务的能力。

项目采用类似记忆树的上下文管理，既能保留上下文内容，也可以节省大量 token。它不是简单“看图点坐标”，而是多轮智能体：模型判断、思考、执行动作、再截图复查，能持续推进任务，而不是只做一次点击。
